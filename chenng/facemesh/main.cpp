...
void detect(const cv::Mat& bgr, const ncnn::Net& net,
            std::vector<FaceObject>& faceobjects)
{
    int w = bgr.cols;
    int h = bgr.rows;
              ...
              std::vector<cv::Point2f>& landmarks)
{
    ...
    ncnn::Mat ncnn_in = ncnn::Mat::from_pixels_resize(
        faceRoiImage.data, ncnn::Mat::PIXEL_BGR2RGB, faceRoiImage.cols,
...
int main(int argc, char** argv)
{
    ...
    face_mesher.load_param(
        (std::string(weight_root) + "/facemesh.param").c_str());
    face_mesher.load_model((std::string(weight_root) + "/facemesh.bin").c_str());

    std::cout << ">>> load model weight done!" << std::endl;

    std::chrono::milliseconds start_time1 = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::system_clock::now().time_since_epoch());

    std::vector<FaceObject> faceobjects;
    detect(bgr, face_detector, faceobjects);

    std::chrono::milliseconds end_time1 = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::system_clock::now().time_since_epoch());
    std::cout << "detect 耗时:"
              << std::chrono::milliseconds(end_time1).count() - std::chrono::milliseconds(start_time1).count()
              << " ms" << std::endl;

    std::cout << ">>> face detect done, found " << faceobjects.size() << " faces!"
              << std::endl;
    ...
    for (size_t i = 0; i < 1; i++)
    {
        ...
        std::chrono::milliseconds start_time2 = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::system_clock::now().time_since_epoch());

        landmark(bgr, face_mesher, obj, pts);

        std::chrono::milliseconds end_time2 = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::system_clock::now().time_since_epoch());
        std::cout << "mesh 耗时:"
                  << std::chrono::milliseconds(end_time2).count() - std::chrono::milliseconds(start_time2).count()
                  << " ms" << std::endl;

        std::cout << ">>> face mesh done!" << std::endl;
        // draw
        for (int i = 0; i < pts.size(); i++)
            cv::circle(bgr, pts[i], 1, cv::Scalar(0, 0, 255), -1);
        for (const auto& contour : contours)
        {
            for (int i = 0; i < contour.size() - 1; i++)
            {
                cv::line(bgr, pts[contour[i]], pts[contour[i + 1]],
                         cv::Scalar(255, 0, 0), 2);
            }
        }
